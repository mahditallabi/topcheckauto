...
const ServicesSection = () => {
const { tString, tArray } = useTranslation('services');


  const services = [
    {
      icon: Battery,
      title: tString('servicesList.electricalSystems.title'),
      description: tString('servicesList.electricalSystems.description'),
      color: "from-blue-500 to-cyan-500",
    },
    {
      icon: Cpu,
      title: tString('servicesList.carDiagnostics.title'),
      description: tString('servicesList.carDiagnostics.description'),
      color: "from-purple-500 to-pink-500",
    },
    {
      icon: Paintbrush,
      title: tString('servicesList.paintInspection.title'),
      description: tString('servicesList.paintInspection.description'),
      color: "from-orange-500 to-red-500",
    },
    {
      icon: Settings,
      title: tString('servicesList.engineTransmission.title'),
      description: tString('servicesList.engineTransmission.description'),
      color: "from-green-500 to-emerald-500",
    },
    {
      icon: Car,
      title: tString('servicesList.carBody.title'),
      description: tString('servicesList.carBody.description'),
      color: "from-indigo-500 to-blue-500",
    },
    {
      icon: Shield,
      title: tString('servicesList.safetySystems.title'),
      description: tString('servicesList.safetySystems.description'),
      color: "from-red-500 to-pink-500",
    },
  ];
  ...
  return (
    <section id="services" className="py-20 section-light">
      <div className="container">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
            <span dangerouslySetInnerHTML={{ __html: tString('title') }} />
          </h2>
          <p className="text-muted-foreground text-lg leading-relaxed">
            {tString('description')}
          </p>
        ...
        {/* Brands */}
        <div className="mt-16 text-center">
          <p className="text-muted-foreground mb-8 text-lg">{tString('brandsSection.title')}</p>
          <div className="flex flex-wrap justify-center items-center gap-8 opacity-60">
           {tArray<string>('brandsSection.brands').map((brand) => (
              <span
                key={brand}
                className="text-lg font-semibold text-foreground/70 hover:text-foreground transition-colors"
              >
